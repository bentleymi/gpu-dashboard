...
def _apply_models_local(path=None) -> None:
    """Per-host model list overrides — untracked, machine-local, never committed.

    models_local.json: {"remove": ["model_id", ...], "add": {"model_id": {entry}}}
    Default: models_local.json next to dashboard.py (env: DASHBOARD_MODELS_LOCAL).
    """
    import json as _json
    import sys as _sys
    if path is None:
        path = os.environ.get(
            "DASHBOARD_MODELS_LOCAL",
            os.path.join(os.path.dirname(os.path.abspath(__file__)), "models_local.json"))
    ...
    try:
        ...
        print(f"[dashboard] {path} unreadable ({e}); using built-in model list",
              file=_sys.stderr)
        return
    removed = sum(1 for mid in local.get("remove", []) if MODELS.pop(mid, None) is not None)
    added = 0
    for mid, entry in (local.get("add") or {}).items():
        if not isinstance(entry, dict) or "name" not in entry or "port" not in entry:
            print(f"[dashboard] models_local add '{mid}' missing name/port; skipped",
                  file=_sys.stderr)
            continue
        MODELS[mid] = entry
        added += 1
    print(f"[dashboard] models_local applied: -{removed} +{added}", file=_sys.stderr)
